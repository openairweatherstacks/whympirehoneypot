...
function sortByAvalanche(left: SimulatedDebt, right: SimulatedDebt) {
  if (right.aprBp !== left.aprBp) return right.aprBp - left.aprBp;
  return left.balanceCents - right.balanceCents;
}

function sortBySnowball(left: SimulatedDebt, right: SimulatedDebt) {
  if (left.balanceCents !== right.balanceCents) return left.balanceCents - right.balanceCents;
  return right.aprBp - left.aprBp;
}

async function getDebtRows(): Promise<DebtAccountRow[]> {
  ...
  `)).rows as unknown as DebtAccountRow[];
}

function buildAccountSummary(row: DebtAccountRow, totalBalanceCents: number) {
  const aprPercent = Number(row.aprBp) / 100;
  const balanceCents = Number(row.balanceCents);
  const estimatedMonthlyInterestCents = Math.round(balanceCents * (aprPercent / 100 / 12));
  const creditLimitCents = Number(row.creditLimitCents);
  const utilizationRatio = creditLimitCents > 0 ? balanceCents / creditLimitCents : null;
  ...
  return {
    ...
    targetPaymentCents: Number(row.targetPaymentCents),
    creditLimitCents,
    utilizationRatio,
    estimatedMonthlyInterestCents,
    shareOfDebt: totalBalanceCents > 0 ? balanceCents / totalBalanceCents : 0
  } satisfies DebtAccountSummary;
}

async function getRecommendedPayment(monthlyMinimumCents: number, monthlyPlannedCents: number) {
...
function simulateStrategy(rows: DebtAccountRow[], monthlyPaymentCents: number, strategy: PayoffStrategy["id"]) {
  const activeAccounts = rows
    .filter((row) => Number(row.balanceCents) > 0)
    .map((row) => ({
      name: row.name,
      balanceCents: Number(row.balanceCents),
      aprBp: Number(row.aprBp),
      minimumPaymentCents: Number(row.minimumPaymentCents)
    }));

  if (activeAccounts.length === 0) {
    return { monthsToDebtFree: 0, totalInterestCents: 0, payoffOrder: [], focusAccount: null };
  }
  ...
  const minimumRequiredCents = activeAccounts.reduce((total, account) => total + account.minimumPaymentCents, 0);
  ...
  while (activeAccounts.some((account) => account.balanceCents > 0) && months < 600) {
    ...
    for (const account of [...activeAccounts].sort(compare)) {
      if (remainingPaymentCents <= 0 || account.balanceCents <= 0) continue;
...
function chooseRecommendedStrategy(strategies: PayoffStrategy[]) {
  ...
  if (!avalanche) return snowball?.id ?? null;
  if (!snowball) return avalanche.id;
...
function buildNegotiationScripts(accounts: DebtAccountSummary[]) {
  ...
  if (highestAprAccount) {
    scripts.push({
      ...
      reason: `${highestAprAccount.name} is carrying ${formatCurrency(highestAprAccount.balanceCents)} at roughly ${formatApr(highestAprAccount.aprPercent)}.`,
  ...
  if (highUtilizationAccount) {
    scripts.push({
      ...
      reason: `${highUtilizationAccount.name} is using about ${Math.round((highUtilizationAccount.utilizationRatio ?? 0) * 100)}% of its credit line, which can pressure both cash flow and utilization.`,
...
export async function saveDebtAccount(input: {
  name: string;
  debtType: string;
  balance: number;
  apr: number;
  ...
  if (!name || !debtType) throw new Error("Debt name and debt type are required.");

  const numericValues = [input.balance, input.apr, input.minimumPayment, input.targetPayment, input.creditLimit ?? 0];
  ...
  const targetPaymentCents = Math.max(minimumPaymentCents, Math.round((input.targetPayment || input.minimumPayment) * 100));
  const creditLimitCents = Math.round((input.creditLimit ?? 0) * 100);
  const aprBp = Math.round(input.apr * 100);

  if (balanceCents <= 0) throw new Error("Balance must be greater than zero.");
  if (minimumPaymentCents <= 0) throw new Error("Minimum payment must be greater than zero.");

  const db = await ensureDb();
  const updatedAt = new Date().toISOString();

  await db.execute({
    sql: `INSERT INTO debt_accounts (
      name, debt_type, balance_cents, apr_bp, minimum_payment_cents,
      target_payment_cents, credit_limit_cents, updated_at
    )
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ...
    args: [name, debtType, balanceCents, aprBp, minimumPaymentCents, targetPaymentCents, creditLimitCents, updatedAt]
  });

  return { name };
}

export async function getDeficitDashboard(): Promise<DeficitDashboard> {
  const rows = await getDebtRows();
  const totalBalanceCents = rows.reduce((total, row) => total + Number(row.balanceCents), 0);
  ...
  const weightedAprPercent =
    totalBalanceCents > 0
      ? accounts.reduce((total, account) => total + account.balanceCents * account.aprPercent, 0) / totalBalanceCents
      : 0;
  const { snapshot, recommendedPaymentCents, latestMonth } = await getRecommendedPayment(
    monthlyMinimumCents,
    monthlyPlannedCents
  );
  ...
  if (highestAprAccount) {
    alerts.push({
      ...
      body: `${highestAprAccount.name} is charging about ${formatApr(highestAprAccount.aprPercent)} on ${formatCurrency(highestAprAccount.balanceCents)}. If you want the fastest mathematical win, this is the balance costing you the most interest every month.`,
  ...
  if (accounts.length > 0 && snapshot.totals.transactionCount > 0 && monthlyPlannedCents > recommendedPaymentCents) {
    alerts.push({
      ...
      body: `Your debt plan totals ${formatCurrency(monthlyPlannedCents)} per month, but a safer payment based on recent surplus looks closer to ${formatCurrency(recommendedPaymentCents)}.`,
      action:
  ...
  if (smallestBalanceAccount && smallestBalanceAccount.balanceCents <= recommendedPaymentCents * 3) {
    alerts.push({
      ...
      body: `${smallestBalanceAccount.name} has the smallest balance at ${formatCurrency(smallestBalanceAccount.balanceCents)}. If motivation matters, snowballing this one away first could free up a monthly payment quickly.`,
  ...
  if (highUtilizationAccount) {
    alerts.push({
      ...
      body: `${highUtilizationAccount.name} is using about ${Math.round((highUtilizationAccount.utilizationRatio ?? 0) * 100)}% of its available credit. That can keep pressure on both score recovery and minimum payments.`,
...
export async function seedDemoDebtAccounts() {
  const db = await ensureDb();
  const existingResult = await db.execute("SELECT COUNT(*) AS count FROM debt_accounts");
  const existing = existingResult.rows[0] as unknown as { count: number };

  if (Number(existing.count) > 0) {
    return { created: false, accountsInserted: Number(existing.count) };
  }

  const starterAccounts = [
    { name: "Rewards Visa", debtType: "Credit Card", balance: 6840, apr: 24.99, minimumPayment: 205, targetPayment: 420, creditLimit: 9000 },
    { name: "Civic Auto Loan", debtType: "Auto Loan", balance: 12400, apr: 6.4, minimumPayment: 318, targetPayment: 318, creditLimit: 0 },
    { name: "Federal Student Loan", debtType: "Student Loan", balance: 8700, apr: 4.8, minimumPayment: 142, targetPayment: 180, creditLimit: 0 }
  ];
  ...
  return { created: true, accountsInserted: starterAccounts.length };
}

export async function answerDebtQuestion(question: string) {
  const normalized = question.toLowerCase();
  const looksDebtRelated =
    normalized.includes("debt") || normalized.includes("loan") || normalized.includes("apr") ||
    normalized.includes("snowball") || normalized.includes("avalanche") ||
    normalized.includes("utilization") || normalized.includes("credit score") ||
    normalized.includes("pay off") || normalized.includes("debt free") ||
    normalized.includes("balance transfer") || normalized.includes("minimum payment") ||
    normalized.includes("negotiat") || normalized.includes("pay first");

  if (!looksDebtRelated) return null;

  const dashboard = await getDeficitDashboard();
  if (dashboard.stats.accounts === 0) {
    return {
      answer: "I don't have any debt accounts saved yet, so I can't rank payoff priorities or project a debt-free date. Add a debt manually or load the sample debt stack first.",
  ...
  const alternateStrategy = dashboard.strategies.find((strategy) => strategy.id !== recommendedStrategy?.id);
  ...
  if (normalized.includes("snowball") || normalized.includes("avalanche")) {
    const interestGapCents = alternateStrategy ? alternateStrategy.totalInterestCents - recommendedStrategy.totalInterestCents : 0;
    ...
    return {
      answer: `${recommendedStrategy.label} is the better fit right now. It points extra cash at ${recommendedStrategy.focusAccount ?? "your highest-priority balance"} first and projects ${
        recommendedStrategy.monthsToDebtFree === null ? "a payoff horizon that still needs a larger payment" : `${recommendedStrategy.monthsToDebtFree} months to debt freedom`
      }. ${alternateStrategy ? `Compared with ${alternateStrategy.label.toLowerCase()}, that is ${Math.abs(monthGap)} month${Math.abs(monthGap) === 1 ? "" : "s"} different and about ${formatCurrency(Math.abs(interestGapCents))} in interest ${interestGapCents >= 0 ? "saved" : "more expensive"}.` : ""}`,
      suggestions: DEBT_SUGGESTIONS
    };
  }

  if (normalized.includes("debt free") || normalized.includes("pay off") || normalized.includes("how long")) {
    return {
      answer:
        recommendedStrategy.monthsToDebtFree === null
          ? `At the current recommended payment of ${formatCurrency(dashboard.stats.recommendedPaymentCents)}, the balances are not paying down fast enough for a clean projection yet. Raising the payment above minimums is the next move.`
          : `If you follow the ${recommendedStrategy.label.toLowerCase()} plan at about ${formatCurrency(recommendedStrategy.monthlyPaymentCents)} per month, you project to be debt free in roughly ${recommendedStrategy.monthsToDebtFree} months with about ${formatCurrency(recommendedStrategy.totalInterestCents)} of additional interest from here.`,
      suggestions: DEBT_SUGGESTIONS
    };
  }
  ...
  if (normalized.includes("apr") || normalized.includes("interest") || normalized.includes("negotiat")) {
    return {
      answer: firstScript
        ? `${highestAprAccount.name} is the account to pressure first because it is charging about ${formatApr(highestAprAccount.aprPercent)}. Use this negotiation angle: ${firstScript.script}`
        : `Your most expensive rate right now is on ${highestAprAccount.name} at about ${formatApr(highestAprAccount.aprPercent)}. That is the first lender worth calling for an APR review or hardship option.`,
      suggestions: ["Which debt should I pay first?", "Should I use avalanche or snowball?", "How long until I am debt free?"]
    };
  }
  ...
  if (normalized.includes("utilization") || normalized.includes("credit score")) {
    return {
      answer: highUtilizationAccount
        ? `${highUtilizationAccount.name} is the utilization problem to solve first at about ${Math.round((highUtilizationAccount.utilizationRatio ?? 0) * 100)}% usage. Getting that account below 30% utilization will usually help faster than spreading tiny payments across every card.`
      ...
      suggestions: ["Which debt should I pay first?", "Can I negotiate a lower APR?", "How long until I am debt free?"]
    };
  }

  return {
    answer: `${recommendedStrategy.label} is the lead plan right now. Start with ${recommendedStrategy.focusAccount ?? highestAprAccount.name}, keep paying at least ${formatCurrency(dashboard.stats.monthlyMinimumCents)} across the rest, and aim for about ${formatCurrency(dashboard.stats.recommendedPaymentCents)} total each month if cash flow holds.`,
    suggestions: DEBT_SUGGESTIONS
  };
}
