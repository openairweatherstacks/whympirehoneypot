...
function normalizeDateWithFallback(value: string, fallbackYear: number) {
  ...
  if (exact) {
    if (/^\d{1,2}[/-]\d{1,2}$/.test(value.trim())) {
      return `${fallbackYear}-${exact.slice(5)}`;
    }
    return exact;
  }
  ...
  const rawYear = monthNameMatch[3]
    ? monthNameMatch[3].length === 2
      ...
      : monthNameMatch[3]
    : String(fallbackYear);
  const normalized = `${monthNameMatch[1]} ${monthNameMatch[2]}, ${rawYear}`;
...
function extractAmountFromLine(line: string) {
  ...
  const preferredMatch =
    [...matches].reverse().find((match) => line.slice((match.index ?? 0) + match[0].length).trim().length <= 14) ??
  ...
  if (tokenHasNegativeMarker || qualifier === "DR") {
    return { amount: -Math.abs(parsed), token, index: preferredMatch.index ?? 0 };
  }

  if (qualifier === "CR" || CREDIT_HINTS.test(line)) {
    return { amount: Math.abs(parsed), token, index: preferredMatch.index ?? 0 };
  }

  return {
    amount: DEBIT_HINTS.test(line) ? -Math.abs(parsed) : -Math.abs(parsed),
    token,
    index: preferredMatch.index ?? 0
  };
}
...
function lineLooksSkippable(line: string) {
  return (
    line.length < 8 ||
    SKIP_LINE_PATTERNS.some((pattern) => pattern.test(line)) ||
    /^[\d\s./-]+$/.test(line) ||
    !/[a-z]/i.test(line)
  );
}
...
export function extractTransactionsFromText(text: string) {
  ...
  for (const line of lines) {
    ...
    const candidate = {
      ...
      merchant,
      category,
      amountCents: Math.round(amountMatch.amount * 100),
      direction,
...
function normalizeTransactionCandidate(candidate: AnthropicTransactionCandidate) {
  ...
  const category =
    direction === "income" ? "Income" : inferCategory(description, String(candidate.category ?? ""));
  ...
  const result: ParsedTransaction = {
    ...
    needsReview,
    confidence,
    classificationNote: needsReview ? "Flagged by Claude as ambiguous" : "Claude extraction"
  };
  return result;
}
...
function parseAnthropicExtraction(text: string): DocumentAnalysis {
  ...
  const normalizedTransactions = (payload.transactions ?? [])
    .map((candidate) => normalizeTransactionCandidate(candidate))
    .filter((candidate): candidate is NonNullable<ReturnType<typeof normalizeTransactionCandidate>> => candidate !== null);
...
// ─── Local extraction (fallback when Anthropic is not configured) ───────────
...
async function analyzeDocumentLocally(input: {
  filename: string;
  mimeType: string;
  buffer: Buffer;
  ...
  if (!normalizedText) {
    return {
      documentKind,
      summary: "The file was processed locally, but no readable text was extracted.",
      extractedText: "",
      notes: [
        input.mimeType === PDF_MIME_TYPE
          ? "PDF text extraction returned no readable content."
          : "Local OCR returned no readable content."
      ],
      transactions: [],
      analysisProvider: provider,
      extractionStatus: "archived-only"
    };
  }
  ...
  return {
    ...
    notes: [
      input.mimeType === PDF_MIME_TYPE
        ? "PDF text was extracted locally with pdf-parse."
        : "Image text was OCRed locally with Tesseract.js."
    ],
    transactions,
    analysisProvider: provider,
...
// ─── Anthropic extraction (primary when configured) ──────────────────────────

async function analyzeDocumentWithAnthropic(input: {
  filename: string;
  mimeType: string;
  buffer: Buffer;
}): Promise<DocumentAnalysis> {
  ...
  const documentBlock =
    input.mimeType === PDF_MIME_TYPE
      ? ({
          type: "document",
          source: { type: "base64", media_type: PDF_MIME_TYPE as "application/pdf", data: base64Data }
        } as const)
      : ({
          type: "image",
          source: {
  ...
  const response = await client.messages.create({
    ...
    max_tokens: 2000,
    system:
      `You are a financial document parser. Return strict JSON only — no markdown, no code fences.
...
function saveHistoricalDocument(input: {
  filename: string;
  mimeType: string;
  documentKind: string;
  source: string;
...
export function deleteDocument(id: number): { deleted: boolean; transactionsRemoved: number } {
  const db = getDb();

  const doc = db
    .prepare("SELECT filename FROM ingestion_documents WHERE id = ?")
    .get(id) as { filename: string } | undefined;

  if (!doc) return { deleted: false, transactionsRemoved: 0 };

  let transactionsRemoved = 0;

  const removeAll = db.transaction(() => {
    // Find matching imports by filename
    const matchingImports = db
      .prepare("SELECT id FROM imports WHERE filename = ?")
      .all(doc.filename) as Array<{ id: number }>;
    ...
    db.prepare("DELETE FROM ingestion_documents WHERE id = ?").run(id);
  });
...
export function getHistoricalDocuments(limit = 8) {
  const db = getDb();
  return db
    .prepare(`
      SELECT
      ...
      LIMIT ?
    `)
    .all(limit) as HistoricalDocument[];
}

export async function ingestUploadedDocument(file: File, member = "joint"): Promise<UploadedDocumentResult> {
  const mimeType = detectMimeType(file);

  // ── CSV: always local, no AI needed ──────────────────────────────────────
  if (mimeType === "text/csv" || file.name.toLowerCase().endsWith(".csv")) {
    const csvText = await file.text();
    const records = parseTransactionsCsv(csvText);

    if (records.length === 0) {
      throw new Error("No transaction rows were detected in that CSV.");
    }

    const result = saveImportedTransactions(file.name, records, "csv", member);
    saveHistoricalDocument({
      filename: file.name,
      mimeType: "text/csv",
      documentKind: "transaction_csv",
      source: "csv",
      extractionStatus: "transactions-imported",
      analysisProvider: "local",
      transactionRowsInserted: result.rowsInserted,
      extractedText: csvText,
      summary: `CSV import completed with ${result.rowsInserted} transaction rows.`,
      notes: []
    });
    ...
    return {
      ...
      summary: `Imported ${result.rowsInserted} rows from CSV.`,
      extractionStatus: "transactions-imported"
    };
  }

  // ── PDF / Image: Anthropic-first, local as fallback ───────────────────────
  ...
  if (aiStatus.liveAnalysisEnabled) {
    // Anthropic is the primary analyzer — it reads every upload directly
    try {
      ...
      // Anthropic failed — fall back to local extraction
      const errorMessage =
        anthropicError instanceof Error ? anthropicError.message : "Anthropic document analysis failed.";
      try {
        ...
        saveHistoricalDocument({
          filename: file.name,
          mimeType,
          documentKind: defaultKind,
          source: "failed",
          extractionStatus: "archived-only",
          analysisProvider: "failed",
          transactionRowsInserted: 0,
          extractedText: "",
          summary: "Both Anthropic and local analysis failed. File archived without extraction.",
          notes: [errorMessage]
        });
        return {
          filename: file.name,
          ...
          mode: "archived-only",
          rowsInserted: 0,
          summary: "Both Anthropic and local analysis failed. File archived historically.",
          extractionStatus: "archived-only"
        };
      }
    }
  } else {
    // No Anthropic key — local only
    try {
      analysis = await analyzeDocumentLocally({ filename: file.name, mimeType, buffer });
    } catch (error) {
      ...
      saveHistoricalDocument({
        ...
        source,
        extractionStatus: "archived-only",
        analysisProvider: "local-failed",
        transactionRowsInserted: 0,
        extractedText: "",
        summary: "Document archived historically, but local analysis could not extract text.",
      ...
      return {
        ...
        mode: "archived-only",
        rowsInserted: 0,
        summary: "Archived historically — local analysis could not extract text from this file.",
        extractionStatus: "archived-only"
      };
    }
  }
  ...
  if (analysis.transactions.length > 0) {
    const result = saveImportedTransactions(file.name, analysis.transactions, source, member);
    rowsInserted = result.rowsInserted;
    mode = "transactions-imported";
    extractionStatus = "transactions-imported";
  }
  ...
  const providerLabel =
    analysis.analysisProvider === "anthropic"
      ? "Claude"
      : analysis.analysisProvider === "pdf-parse"
        ? "Local PDF parser"
        : "Local OCR";
  ...
  const summary =
    analysis.summary ||
    (rowsInserted > 0
      ? `${providerLabel} extracted ${rowsInserted} transaction rows from the uploaded document.`
      : `${providerLabel} reviewed the document and archived it historically, but did not find reliable transaction rows.`);

  saveHistoricalDocument({
    filename: file.name,
    mimeType,
    documentKind: analysis.documentKind || defaultKind,
    source,
    extractionStatus,
    analysisProvider: analysis.analysisProvider,
    transactionRowsInserted: rowsInserted,
    extractedText: analysis.extractedText,
    summary: normalizeTextPreview(summary, "Document processed."),
