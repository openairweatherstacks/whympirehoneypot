...
function mapRow(row: RecurringRow): RecurringExpense {
  return {
    ...
    monthlyEquivalentCents: monthlyEquivalent(row.amountCents, row.frequency)
  };
}
...
export function addRecurringExpense(input: {
  name: string;
  merchant?: string;
  category: string;
  amount: number;
  ...
  const result = db
    .prepare(`
      INSERT INTO recurring_expenses (name, merchant, category, amount_cents, frequency, source, active, member, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?, ?)
    `)
    .run(name, merchant, category, amountCents, input.frequency, source, member, now, now);

  const row = db
    .prepare(`
      SELECT id, name, merchant, category,
        amount_cents AS amountCents, frequency, source, active, member,
        created_at AS createdAt, updated_at AS updatedAt
      FROM recurring_expenses WHERE id = ?
    `)
    .get(result.lastInsertRowid) as RecurringRow;
...
export function deleteRecurringExpense(id: number): boolean {
  const db = getDb();
  const result = db.prepare("DELETE FROM recurring_expenses WHERE id = ?").run(id);
...
export function updateRecurringExpense(
  ...
  const amountCents = input.amount ? Math.round(input.amount * 100) : existing.amountCents;
  ...
  db.prepare(`
    ...
    WHERE id = ?
  `).run(name, category, amountCents, frequency, now, id);
  ...
  const row = db
    .prepare(`
      ...
      FROM recurring_expenses WHERE id = ?
    `)
    .get(id) as RecurringRow;

  return mapRow(row);
}

export function detectRecurringFromTransactions(): DetectedCandidate[] {
  const db = getDb();
  ...
  const detected = db
    .prepare(`
      ...
      LIMIT 20
    `)
    .all() as { merchant: string; category: string; avgAmountCents: number; activeMonths: number }[];

  const saved = db
    .prepare("SELECT merchant FROM recurring_expenses")
  ...
  return detected.map((row) => ({
    merchant: row.merchant,
    category: row.category,
    avgAmountCents: row.avgAmountCents,
